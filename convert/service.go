...
func Service(
	...
	endpoint, err := convertEndpointSpec(deploy.EndpointMode, service.Ports)
	if err != nil {
		return swarm.ServiceSpec{}, err
	}

	mode, err := convertDeployMode(deploy.Mode, deploy.Replicas)
	if err != nil {
		return swarm.ServiceSpec{}, err
	}
	...
	restartPolicy, err := convertRestartPolicy(
		service.Restart, deploy.RestartPolicy)
	if err != nil {
		return swarm.ServiceSpec{}, err
	}
	...
	healthcheck, err := convertHealthcheck(healthCheck)
	if err != nil {
		return swarm.ServiceSpec{}, err
	}
	...
	if credentialSpec == nil {
		credentialSpec = &composego.CredentialSpecConfig{}
	}
	privileges.CredentialSpec, err = convertCredentialSpec(
		namespace, *credentialSpec, configs,
	)
	if err != nil {
		return swarm.ServiceSpec{}, err
	}
	...
	serviceSpec := swarm.ServiceSpec{
		Annotations: swarm.Annotations{
			Name:   name,
			Labels: AddStackLabel(namespace, deploy.Labels),
		},
		TaskTemplate: swarm.TaskSpec{
			ContainerSpec: &swarm.ContainerSpec{
				...
				User:            service.User,
				Mounts:          mounts,
				StopGracePeriod: composego.ConvertDurationPtr(service.StopGracePeriod),
			...
			Placement: &swarm.Placement{
				...
				Preferences: getPlacementPreference(deploy.Placement.Preferences),
				MaxReplicas: deploy.Placement.MaxReplicas,
			},
		},
		EndpointSpec:   endpoint,
		Mode:           mode,
		UpdateConfig:   convertUpdateConfig(deploy.UpdateConfig),
		RollbackConfig: convertUpdateConfig(deploy.RollbackConfig),
	}
...
func getPlacementPreference(preferences []composego.PlacementPreferences) []swarm.PlacementPreference {
...
func convertServiceNetworks(
	...
	networkConfigs composego.Networks,
	namespace Namespace,
	name string,
...
func convertServiceSecrets(
	client apiclient.SecretAPIClient,
	namespace Namespace,
	secrets []composego.ServiceSecretConfig,
	secretSpecs composego.Secrets,
) ([]*swarm.SecretReference, error) {
	refs := []*swarm.SecretReference{}

	lookup := func(key string) (composego.FileObjectConfig, error) {
		secretSpec, exists := secretSpecs[key]
		if !exists {
			return composego.FileObjectConfig{}, errors.Errorf("undefined secret %q", key)
		}
		return composego.FileObjectConfig(secretSpec), nil
	}
	for _, secret := range secrets {
		obj, err := convertFileObject(namespace, composego.FileReferenceConfig(secret), lookup)
		if err != nil {
			return nil, err
		}
...
func convertServiceConfigObjs(
	client apiclient.ConfigAPIClient,
	namespace Namespace,
	service composego.ServiceConfig,
	configSpecs composego.Configs,
	...
	lookup := func(key string) (composego.FileObjectConfig, error) {
		...
		if !exists {
			return composego.FileObjectConfig{}, errors.Errorf("undefined config %q", key)
		}
		return composego.FileObjectConfig(configSpec), nil
	}
	for _, config := range service.Configs {
		obj, err := convertFileObject(namespace, composego.FileReferenceConfig(config), lookup)
		if err != nil {
			return nil, err
		}
...
func convertFileObject(
	namespace Namespace,
	config composego.FileReferenceConfig,
	lookup func(key string) (composego.FileObjectConfig, error),
...
func convertExtraHosts(extraHosts composego.HostsList) []string {
...
func convertHealthcheck(healthcheck *composego.HealthCheckConfig) (*container.HealthConfig, error) {
	if healthcheck == nil {
		return nil, nil
	}
	var (
...
func convertRestartPolicy(restart string, source *composego.RestartPolicy) (*swarm.RestartPolicy, error) {
	...
	return &swarm.RestartPolicy{
		...
		Delay:       composego.ConvertDurationPtr(source.Delay),
		MaxAttempts: source.MaxAttempts,
		Window:      composego.ConvertDurationPtr(source.Window),
	}, nil
}

func convertUpdateConfig(source *composego.UpdateConfig) *swarm.UpdateConfig {
	if source == nil {
		return nil
	}
	parallel := uint64(1)
...
func convertResources(source composego.Resources) (*swarm.ResourceRequirements, error) {
...
func convertEndpointSpec(endpointMode string, source []composego.ServicePortConfig) (*swarm.EndpointSpec, error) {
	portConfigs := []swarm.PortConfig{}
	for _, port := range source {
		publishedPort, err := strconv.Atoi(port.Published)
		if err != nil {
			return nil, errors.Errorf("published port: '%s' must be coercible to integer.", port.Published)
		}
...
func convertCredentialSpec(namespace Namespace, spec composego.CredentialSpecConfig, refs []*swarm.ConfigReference) (*swarm.CredentialSpec, error) {
	var o []string
...
func convertUlimits(origUlimits map[string]*composego.UlimitsConfig) []*units.Ulimit {
