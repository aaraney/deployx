...
func deployCompose(ctx context.Context, dockerCli command.Cli, opts options.Deploy, config *composego.Config) error {
...
func getServicesDeclaredNetworks(serviceConfigs composego.Services) map[string]struct{} {
...
func validateExternalNetworks(ctx context.Context, client apiclient.NetworkAPIClient, externalNetworks []string) error {
	for _, networkName := range externalNetworks {
		...
		case apiclient.IsErrNotFound(err):
			return errors.New(fmt.Sprintf("network %q is declared as external, but could not be found. You need to create a swarm-scoped network before the stack is deployed", networkName))
		case err != nil:
			return err
		case network.Scope != "swarm":
			return errors.New(fmt.Sprintf("network %q is declared as external, but it is not in the right scope: %q instead of \"swarm\"", networkName, network.Scope))
		}
	}
	return nil
}
